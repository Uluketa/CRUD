...
using System.Data;
using Projeto_Vendas_ETEC.br.com.projetos.views;
...
namespace Projeto_Vendas_ETEC.br.com.projetos.dao
{
    public class FuncionarioDAO
    {
        ...
        public void EfetuarLogin(string email, string senha)
        {
            try
            {
                string sql = @"select * from tb_funcionarios where tb_funcionarios_email = @email and tb_funcionarios_senha = @senha";
                ...
                if (rs.Read())
                {
                    string nivel = rs.GetString("TB_FUNCIONARIOS_NIVEL_ACESSO");
                    string nome = rs.GetString("TB_FUNCIONARIOS_NOME");

                    Frmmenu telamenu = new Frmmenu();

                    if (nivel.Equals("AdministradorA"))
                    {
                    ...
                    else if (nivel.Equals("AdministradorB"))
                    {
                        MessageBox.Show("Bem vindo ao sistema " + nome);
                        telamenu.toolStripMenuItem22.Visible = false;

                        telamenu.ShowDialog();
                    }

                    else if (nivel.Equals("AdministradorC"))
                    {
                        MessageBox.Show("Bem vindo ao sistema " + nome);
                        telamenu.toolStripMenuItem22.Visible = false;
                        telamenu.toolStripMenuItem19.Visible = false;

                        telamenu.ShowDialog();
                    }

                    else if (nivel.Equals("AdministradorD"))
                    {
                        MessageBox.Show("Bem vindo ao sistema " + nome);
                        telamenu.toolStripMenuItem22.Visible = false;
                        telamenu.toolStripMenuItem16.Visible = false;
                        telamenu.toolStripMenuItem19.Visible = false;
                    ...
                    else if (nivel.Equals("Usuário"))
                    {
                        ...
                        telamenu.toolStripMenuItem13.Visible = false;
                        telamenu.toolStripMenuItem22.Visible = false;
                        telamenu.toolStripMenuItem16.Visible = false;
                        telamenu.toolStripMenuItem19.Visible = false;
            ...
            {
                MessageBox.Show("Ops, ocorreu o erro:: "+ex);
            }
        }
